...
func MakeClientHandshakeFinished(clientWriteKey, clientIV, verifyData []byte, tlsVersion [2]byte, seqNum byte) (ClientHandshakeFinished, error) {
	...
	encryptedContent, err := cryptoHelpers.Encrypt(clientWriteKey, clientIV, plaintext, additionalData)
